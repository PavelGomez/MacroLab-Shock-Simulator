...
function updateScenarioCard(modelKey,context){
  ...
  setHTML(`${modelKey}-scenario-next`,scenarioNextStep(modelKey,baseScenario,meta,context,shock));
...
const INST_RESULTS = {
  'petroleo_CHL':{absorcion:'TC + inflación transitoria',trayectoria:'depreciación moderada contenida por BC creíble',persistencia:'media',institucion:'Credibilidad BCCh',dato:['IPC energía','CLP/USD'],advertencia:'El TC flexible amortigua pero también transmite el shock vía pass-through. No es neutral.',modelo:'oada',contraste:'El modelo muestra P↑ Y↓. Esta config agrega: ajuste rápido porque el BC ancla expectativas y el TC absorbe parte del shock.'},
  'petroleo_FRG':{absorcion:'Precios + expectativas',trayectoria:'depreciación inflacionaria con segunda vuelta intensa',persistencia:'alta',institucion:'Prima de riesgo soberano',dato:['Spread EMBI','Expectativas inflacionarias'],advertencia:'En marcos frágiles el shock de oferta puede generar crisis de confianza independiente del shock original.',modelo:'oada',contraste:'El modelo muestra P↑ Y↓. Esta config agrega: segunda vuelta inflacionaria porque las expectativas se desanclan y la prima de riesgo sube.'},
  'petroleo_RIG':{absorcion:'Actividad + reservas',trayectoria:'contracción sin absorción cambiaria, salarios reales caen',persistencia:'alta',institucion:'Reservas internacionales',dato:['Reservas brutas','Salarios reales'],advertencia:'La rigidez cambiaria hace invisible el ajuste del TC pero lo traslada a desempleo y actividad.',modelo:'oada',contraste:'El modelo muestra P↑ Y↓. Esta config agrega: mayor caída de Y porque sin TC flexible el ajuste recae sobre salarios y actividad.'},
  'petroleo_CRD':{absorcion:'Precios parcialmente',trayectoria:'inflación transitoria y acotada, anclaje preservado',persistencia:'baja',institucion:'Fondo de estabilización + BC',dato:['Breakeven inflación','Producción industrial'],advertencia:'La baja persistencia no es inmunidad: es credibilidad de la respuesta. Sin ella, el perfil sería distinto.',modelo:'oada',contraste:'El modelo muestra P↑ Y↓. Esta config agrega: convergencia rápida porque el fondo de estabilización y la credibilidad del BC reducen la persistencia.'},
  'cobre_CHL':{absorcion:'TC + ajuste fiscal gradual',trayectoria:'depreciación moderada, contracción fiscal ordenada',persistencia:'media',institucion:'Regla de balance estructural',dato:['Precio cobre','CLP/USD'],advertencia:'El ajuste fiscal chileno no es automático: requiere que la regla sea creíble y el espacio fiscal exista.',modelo:'islmbp',contraste:'El modelo muestra Y↓ E↑. Esta config agrega: ajuste fiscal gradual y creíble que evita que el shock cambiario se vuelva crisis.'},
  'cobre_FRG':{absorcion:'TC + prima de riesgo',trayectoria:'espiral cambiaria-inflacionaria, dudas sobre sostenibilidad fiscal',persistencia:'muy-alta',institucion:'Credibilidad fiscal + BC',dato:['CDS soberano','Spread EMBI'],advertencia:'La caída de un commodity puede gatillar una crisis mayor que el shock original si la credibilidad es frágil.',modelo:'islmbp',contraste:'El modelo muestra Y↓ E↑. Esta config agrega: espiral cambiaria-fiscal porque la depreciación deteriora la credibilidad y el spread sube.'},
  'cobre_RIG':{absorcion:'Actividad + reservas',trayectoria:'recesión directa sin TC que absorba, presión bancaria',persistencia:'alta',institucion:'Nivel de reservas internacionales',dato:['Reservas','Balanza de pagos'],advertencia:'Con TC fijo, la pérdida de términos de intercambio se convierte íntegramente en caída de actividad.',modelo:'islmbp',contraste:'El modelo muestra Y↓. Esta config agrega: recesión más profunda porque sin TC que deprecie, la pérdida de ingresos es íntegra sobre actividad.'},
  'cobre_CRD':{absorcion:'TC + buffer fiscal',trayectoria:'ajuste contenido, fondo activa gasto contracíclico',persistencia:'baja',institucion:'Fondo soberano de estabilización',dato:['Precio cobre','Gasto del fondo'],advertencia:'El buffer reduce la transmisión pero el shock llega igual. El riesgo es subestimar el impacto sin buffer.',modelo:'islmbp',contraste:'El modelo muestra Y↓ E↑. Esta config agrega: el fondo soberano actúa como amortiguador fiscal, reduciendo el traslado a actividad.'},
  'pandemia_CHL':{absorcion:'DA cae + TC deprecia',trayectoria:'caída de Y compensada parcialmente por gasto creíble',persistencia:'media',institucion:'Espacio fiscal + credibilidad BCCh',dato:['IMACEC','Balance fiscal'],advertencia:'La respuesta contracíclica de Chile fue posible porque había espacio fiscal previo acumulado.',modelo:'oada',contraste:'El modelo muestra DA↓ OA↓. Esta config agrega: respuesta contracíclica creíble posible porque había espacio fiscal acumulado.'},
  'pandemia_FRG':{absorcion:'DA cae + presión cambiaria',trayectoria:'caída de Y con riesgo soberano alto, financiamiento condicionado',persistencia:'alta',institucion:'Prima de riesgo soberano',dato:['Spread soberano','Reservas'],advertencia:'Sin acceso a mercados, la respuesta fiscal frente a pandemia puede agravar la prima de riesgo.',modelo:'oada',contraste:'El modelo muestra DA↓ OA↓. Esta config agrega: limitación de la respuesta fiscal por prima de riesgo alta y acceso a mercados condicionado.'},
  'pandemia_RIG':{absorcion:'DA cae + reservas',trayectoria:'recesión profunda, salarios reales lentos, recuperación muy lenta',persistencia:'muy-alta',institucion:'Reservas + sistema bancario',dato:['Balance de pagos','Crédito interno'],advertencia:'Con TC fijo el ajuste recae sobre empleo y salarios, que son mucho más rígidos que el tipo de cambio.',modelo:'oada',contraste:'El modelo muestra DA↓ OA↓. Esta config agrega: recesión más profunda y lenta porque sin TC y sin espacio fiscal el ajuste recae sobre empleo.'},
  'pandemia_CRD':{absorcion:'DA cae + buffer activo',trayectoria:'recuperación más rápida, menor cicatrización del tejido productivo',persistencia:'baja',institucion:'Fondo soberano + regla fiscal contracíclica',dato:['IMACEC','Gasto del fondo'],advertencia:'La ventaja institucional no elimina el shock: reduce su profundidad y acorta la duración del ajuste.',modelo:'oada',contraste:'El modelo muestra DA↓ OA↓. Esta config agrega: recuperación más rápida porque el buffer fiscal permite respuesta inmediata sin comprometer credibilidad.'},
  'fiscalExpand_CHL':{absorcion:'TC + tasa',trayectoria:'expansión moderada con fuga cambiaria parcial, crowding-out parcial',persistencia:'media',institucion:'Regla fiscal',dato:['TPM','CLP/USD'],advertencia:'IS-LM predice expansión; IS-LM-BP muestra la fuga cambiaria parcial. Ambos modelos son necesarios.',modelo:'islm',contraste:'El modelo muestra Y↑ i↑. Esta config agrega: fuga cambiaria parcial vía IS-LM-BP que modera el impulso.'},
  'fiscalExpand_FRG':{absorcion:'Tasa + expectativas inflacionarias',trayectoria:'expansión con riesgo de dominancia fiscal',persistencia:'alta',institucion:'Autonomía efectiva del BC',dato:['Expectativas inflación 2 años','Déficit primario'],advertencia:'La expansión puede parecer exitosa de corto plazo y ser insostenible de mediano si el BC acomoda.',modelo:'islm',contraste:'El modelo muestra Y↑ i↑. Esta config agrega: riesgo de dominancia fiscal si el BC acomoda; corto plazo expansivo, mediano plazo frágil.'},
  'fiscalExpand_RIG':{absorcion:'Actividad (multiplicador alto)',trayectoria:'impulso fuerte de corto plazo sin fuga cambiaria',persistencia:'alta',institucion:'Sostenibilidad de deuda sin TC como válvula',dato:['Nivel de deuda','Reservas'],advertencia:'Con TC fijo la política fiscal es más potente de corto plazo pero la insostenibilidad llega más rápido.',modelo:'islm',contraste:'El modelo muestra Y↑. Esta config agrega: multiplicador alto de corto plazo sin fuga cambiaria, pero insostenibilidad más rápida.'},
  'fiscalExpand_CRD':{absorcion:'Tasa + credibilidad',trayectoria:'expansión acotada, BC reacciona rápido, poca inflación adicional',persistencia:'baja',institucion:'Credibilidad del BC',dato:['Brecha de producto','Balance primario'],advertencia:'Un BC muy creíble puede neutralizar casi completamente el impulso fiscal vía expectativas.',modelo:'islm',contraste:'El modelo muestra Y↑ i↑. Esta config agrega: el BC creíble neutraliza buena parte del impulso vía expectativas y tasas.'},
  'fiscalContract_CHL':{absorcion:'Actividad + tasa',trayectoria:'contracción ordenada, algo de alivio externo',persistencia:'media',institucion:'Regla fiscal',dato:['IMACEC','TPM'],advertencia:'La contracción fiscal puede aliviar presión externa y ayudar a credibilidad si el BC la acompaña.',modelo:'islm',contraste:'El modelo muestra Y↓ i↓. Esta config agrega: ajuste ordenado y creíble que puede aliviar presión externa.'},
  'fiscalContract_FRG':{absorcion:'Actividad + confianza',trayectoria:'contracción con riesgo de señal negativa sobre sostenibilidad',persistencia:'alta',institucion:'Prima de riesgo',dato:['CDS soberano','Brecha de producto'],advertencia:'En marcos frágiles, ajustar el gasto puede ser leído como señal de crisis, no de disciplina.',modelo:'islm',contraste:'El modelo muestra Y↓ i↓. Esta config agrega: riesgo de que el mercado lea el ajuste como señal de crisis, no de disciplina.'},
  'fiscalContract_RIG':{absorcion:'Actividad (contracción fuerte)',trayectoria:'ajuste severo sin válvula cambiaria',persistencia:'alta',institucion:'Nivel de deuda + reservas',dato:['PIB','Reservas'],advertencia:'La contracción fiscal con TC fijo impacta directamente en actividad sin amortiguador externo.',modelo:'islm',contraste:'El modelo muestra Y↓. Esta config agrega: caída de actividad más intensa sin válvula cambiaria que amortigüe.'},
  'fiscalContract_CRD':{absorcion:'Actividad moderada + tasa',trayectoria:'ajuste ordenado, expectativas estables',persistencia:'baja',institucion:'Regla fiscal + credibilidad',dato:['Brecha de producto','Balance estructural'],advertencia:'La credibilidad permite que la contracción sea ordenada y no genere espiral de expectativas negativas.',modelo:'islm',contraste:'El modelo muestra Y↓ i↓. Esta config agrega: ajuste ordenado con expectativas estables; credibilidad preservada.'},
  'monetaryContract_CHL':{absorcion:'Tasa + inversión',trayectoria:'enfriamiento gradual, ancla de expectativas preservada',persistencia:'media',institucion:'Credibilidad BCCh',dato:['TPM','Inversión privada'],advertencia:'El endurecimiento es más eficaz y menos costoso cuando las expectativas ya están ancladas.',modelo:'islm',contraste:'El modelo muestra Y↓ i↑. Esta config agrega: transmisión eficaz porque las expectativas ya están ancladas.'},
  'monetaryContract_FRG':{absorcion:'Tasa + actividad (con riesgo)',trayectoria:'enfriamiento con posible fuga de capitales y presión cambiaria',persistencia:'alta',institucion:'Autonomía efectiva del BC',dato:['Spread soberano','CLP/USD análogo'],advertencia:'En marcos frágiles, subir la tasa puede percibirse como señal de crisis más que de disciplina.',modelo:'islm',contraste:'El modelo muestra Y↓ i↑. Esta config agrega: riesgo de que la suba de tasa sea leída como señal de crisis más que de disciplina.'},
  'monetaryContract_RIG':{absorcion:'Sin efecto monetario autónomo',trayectoria:'sin canal propio, ajuste depende de política fiscal',persistencia:'alta',institucion:'Política fiscal (único instrumento disponible)',dato:['Balance fiscal','Reservas'],advertencia:'Con TC fijo y alta movilidad de capitales la política monetaria propia es ineficaz o inexistente.',modelo:'islm',contraste:'El modelo muestra Y↓ i↑. Esta config agrega: sin canal propio, la política monetaria no existe; solo opera la fiscal.'},
  'monetaryContract_CRD':{absorcion:'Tasa + expectativas',trayectoria:'transmisión rápida y creíble, menor sobre-ajuste necesario',persistencia:'baja',institucion:'Credibilidad del BC',dato:['Expectativas inflación','Brecha de producto'],advertencia:'La credibilidad permite hacer menos para lograr más: menor suba de tasa, mismo efecto de anclaje.',modelo:'islm',contraste:'El modelo muestra Y↓ i↑. Esta config agrega: menor suba de tasa necesaria para el mismo efecto; credibilidad hace el trabajo.'},
  'globalRecession_CHL':{absorcion:'TC + exportaciones',trayectoria:'depreciación, contracción exportadora, algo de respuesta fiscal',persistencia:'media',institucion:'Apertura externa + regla fiscal',dato:['Exportaciones','CLP/USD'],advertencia:'El shock externo entra por exportaciones y tipo de cambio simultáneamente. IS-LM-BP captura ambos canales.',modelo:'islmbp',contraste:'El modelo muestra Y↓ E↑. Esta config agrega: depreciación como amortiguador parcial de la caída de demanda externa.'},
  'globalRecession_FRG':{absorcion:'TC + prima de riesgo',trayectoria:'depreciación inflacionaria, contagio financiero, acceso a deuda comprometido',persistencia:'muy-alta',institucion:'Prima de riesgo soberano',dato:['Spread EMBI','Acceso a mercados'],advertencia:'En marcos frágiles una recesión global puede cortar el financiamiento externo y amplificar el shock.',modelo:'islmbp',contraste:'El modelo muestra Y↓ E↑. Esta config agrega: contagio financiero que corta el acceso a mercados y amplifica el shock.'},
  'globalRecession_RIG':{absorcion:'Actividad + reservas',trayectoria:'recesión directa, sin TC que absorba, presión sobre reservas',persistencia:'muy-alta',institucion:'Nivel de reservas',dato:['Reservas brutas','Balance de pagos'],advertencia:'Sin tipo de cambio flexible, la recesión global se transmite íntegramente como contracción interna.',modelo:'islmbp',contraste:'El modelo muestra Y↓. Esta config agrega: sin TC que absorba, la caída de demanda externa se transmite íntegra a actividad.'},
  'globalRecession_CRD':{absorcion:'TC + buffer',trayectoria:'impacto moderado, buffer fiscal activo, expectativas estables',persistencia:'baja',institucion:'Fondo soberano + apertura externa',dato:['Exportaciones','Gasto del fondo'],advertencia:'El buffer no elimina el shock externo pero reduce la amplitud del ciclo y protege el empleo.',modelo:'islmbp',contraste:'El modelo muestra Y↓ E↑. Esta config agrega: el buffer fiscal amortigua la caída y las expectativas no se desanclan.'},
  'guerra_CHL':{absorcion:'TC + costos + expectativas',trayectoria:'depreciación + inflación de costos + incertidumbre',persistencia:'media',institucion:'Credibilidad BCCh + pass-through',dato:['IPC energía','CLP/USD'],advertencia:'El shock de guerra es mixto: OA y DA se mueven a la vez. Ningún modelo lo captura solo.',modelo:'oada',contraste:'El modelo muestra P↑ Y↓. Esta config agrega: depreciación más pass-through moderado por credibilidad; inflación transitoria.'},
  'guerra_FRG':{absorcion:'Precios + expectativas + prima',trayectoria:'espiral inflacionaria-cambiaria, segunda vuelta intensa',persistencia:'muy-alta',institucion:'Prima de riesgo + BC',dato:['Expectativas inflación','Spread EMBI'],advertencia:'En marcos frágiles el componente de confianza amplifica el shock energético hasta hacerlo fiscal.',modelo:'oada',contraste:'El modelo muestra P↑ Y↓. Esta config agrega: segunda vuelta intensa porque el componente de confianza amplifica el shock energético.'},
  'guerra_RIG':{absorcion:'Actividad + reservas',trayectoria:'sin absorción cambiaria, costos suben, actividad cae',persistencia:'alta',institucion:'Reservas + política fiscal',dato:['Reservas','Salarios reales'],advertencia:'La guerra golpea los costos; sin TC para depreciar, el ajuste recae sobre empleo y reservas.',modelo:'oada',contraste:'El modelo muestra P↑ Y↓. Esta config agrega: sin TC, los costos energéticos se absorben íntegramente en actividad y salarios.'},
  'guerra_CRD':{absorcion:'Precios parcialmente',trayectoria:'inflación transitoria, anclaje preservado, respuesta fiscal rápida',persistencia:'baja',institucion:'Fondo de estabilización + credibilidad BC',dato:['Breakeven inflación','Precio energía local'],advertencia:'La respuesta coordinada fiscal-monetaria amortigua el shock. El riesgo es subestimar la duración.',modelo:'oada',contraste:'El modelo muestra P↑ Y↓. Esta config agrega: respuesta coordinada fiscal-monetaria que amortigua persistencia; convergencia más rápida.'}
};
...
function lentesCardHTML(resultado, configId, shockId) {
  ...
  return `
    <div class="atlas-card card ${colorClass}"
      ...
      ${resultado.contraste ? `
      <div style="background:var(--navy);color:#fff;padding:10px 12px;
                  border-radius:8px;margin-top:10px;font-size:.85rem">
        <strong style="color:rgba(255,255,255,.7);font-size:.75rem;
        text-transform:uppercase;letter-spacing:.06em">Lectura cruzada</strong><br>
        ${resultado.contraste}
      </div>` : ''}
      ${mapeo.tab ? `
      <div style="position:absolute;bottom:14px;right:14px;text-align:right">
        <button class="route-btn lentes-model-btn"
                data-tab="${mapeo.tab}"
                data-shock="${mapeo.shockKey}"
                style="font-size:.82rem;margin:0">
          → equilibrio formal en ${tabLabel}
        </button>
        <div style="font-size:.78rem;color:var(--muted);margin-top:4px;max-width:220px">
          El gráfico muestra el equilibrio estático. La trayectoria
          diferenciada está en la tarjeta.
        </div>
      </div>` : ''}
    </div>`;
}
